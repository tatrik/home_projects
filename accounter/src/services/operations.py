...
class OperationsService:
    ...
    def _get(self, user_id: int, operation_id: int) -> Type[Operation]:
        operation = (
            self.session
    ...
    def get_list(self, user_id: int, kind: Optional[OperationKind] = None) -> List[Type[Operation]]:
        query = (
            self.session
            .query(tables.Operation)
    ...
    def update(self, user_id: int, operation_id: int, operation_data: OperationUpdate) -> Type[Operation]:
        operation = self._get(user_id=user_id, operation_id=operation_id)
